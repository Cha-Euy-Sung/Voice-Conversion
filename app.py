...
import gender_identification.gender as gender
import shutil
...
@app.route('/wrong_gender')
def wrong_gender():
    return render_template('wrong_gender.html')
...
def record():
    if request.method == 'POST' :
        ...
        print('## gender identification begin ##')
        user_gender = gender.test_prediction()
        print('## user gender: ',user_gender,'\n')
        print('Male?',user_gender=='Male')
        if user_gender == 'Female':
            s2s.converting()
        elif user_gender == 'Male':
            s2s.converting()
            print("Get in Men's room")
            #return redirect(url_for('wrong_gender'))
        else:
            pass
